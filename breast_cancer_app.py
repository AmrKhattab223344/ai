...
The dataset contains 30 features computed from digitized images of fine needle aspirate (FNA) of breast mass. The target variable is a binary classification indicating whether the tumor is malignant (encoded as 0) or benign (encoded as 1).
...
feature_importance = pd.DataFrame({'Feature': data.feature_names, 'Importance': model.feature_importances_})
